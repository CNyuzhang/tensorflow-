...
def train(X, Y):
    ...
    learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE,#基础的学习率，在此基础上进行递减
                                               global_step,#迭代的轮数
                                               dataset_size/BATCH_SIZE,#所有的数据得到训练所需要的轮数
    ...
    with tf.Session() as sess:
        ...
        print sess.run(biaes1)
        print sess.run(biaes2)
        ...
        for i in range(TRAINING_STEPS):
            ...
            _,loss_value, step = sess.run([train_op, loss, global_step], feed_dict={x: X[start:end], y_: Y[start:end]})
            if i%1000==0:
...
def main(argv=None):
    ...
    #通过随机数生成一个模拟数据集
    rdm = RandomState(1)        #１代表随机数种子，只要种子相同，产生的随机数序列就相同
